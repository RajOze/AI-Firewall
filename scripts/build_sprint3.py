...
from backend.ai.schemas import AdvisoryAction, ThreatSeverity
...
Safety guardrails to protect OS stability and network infrastructure.
"""
from typing import Final
import ipaddress

# Windows Core Operating System Binaries (Must NEVER be blocked)
...
# Network Loopback & Infrastructure Addresses
...
class PolicyEngine:
    ...
    def evaluate_advisory(self, advisory: ThreatAdvisory) -> PolicyDecision:
        ...
        # If threat does not meet threshold -> MONITOR_ONLY
...
from backend.firewall.models import FirewallAction, FirewallDirection, FirewallProtocol, FirewallRuleCreate
from backend.firewall.service import FirewallService
    ...
    async def evaluate_and_enforce(self, advisory: ThreatAdvisory) -> PolicyDecision:
        ...
        if decision.action == PolicyAction.ENFORCE_BLOCK:
            ...
            rule_create = FirewallRuleCreate(
                ...
                remote_port=str(decision.destination_port) if decision.destination_port else None,
            ...
            try:
                self.firewall_service.add_rule(rule_create)
...
from backend.app.dependencies.firewall import get_firewall_service
from backend.firewall.service import FirewallService
from backend.policy.engine import PolicyEngine
from backend.policy.enforcer import AutonomousEnforcer
...
# 8. tests/unit/policy/test_policy_engine.py
...
def test_whitelist_detection_system_critical():
    """Verify system-critical OS processes are identified and protected."""
...
def test_whitelist_detection_ip_and_loopback():
    """Verify loopback, local, and multicast IPs are whitelisted."""
...
def test_policy_engine_protects_system_critical_process():
    """Verify policy engine will NEVER block a system critical process even for CRITICAL threat."""
    engine = PolicyEngine()
    advisory = ThreatAdvisory(
...
def test_policy_engine_protects_whitelisted_ip():
    """Verify policy engine will NEVER block loopback or infrastructure IP."""
    engine = PolicyEngine()
    advisory = ThreatAdvisory(
...
def test_policy_engine_full_autonomous_block_success():
    """Verify policy engine mandates ENFORCE_BLOCK for high severity and high confidence."""
...
def test_policy_engine_semi_automatic_mode_queues_for_approval():
    """Verify SEMI_AUTOMATIC mode queues mutations for approval rather than blocking immediately."""
...
def test_policy_engine_cooldown_and_rate_limiting():
    """Verify repetitive attacks on same target trigger cooldown rate limiting."""
    ...
    # Immediate second attempt on same target -> Cooldown
...
async def test_autonomous_enforcer_executes_firewall_rule():
    """Verify AutonomousEnforcer invokes FirewallService when policy mandates ENFORCE_BLOCK."""
